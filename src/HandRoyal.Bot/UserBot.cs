...
public sealed class UserBot(BotOptions options, ILogger<UserBot> logger)
    : BotBase(options, logger)
{
    ...
    protected override void OnJobUpdated(JobEventArgs e)
    {
        if (e.JobInfo.Type == typeof(CreateUserJob) && e.JobInfo.IsCompletedSuccessfully)
        {
            _isCreated = true;
        }
